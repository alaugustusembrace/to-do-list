...
async function getProjects() {
  ...
  for (let i = 1; i < projects.length; i++) {
    ...
    newProjectDivision.addEventListener("click", async () => {
      listArea.innerHTML = "";

      const newTasksRes = await fetch(
        `http://localhost:5000/api/projects/${projects[i]._id}`,
      );
      const newTasks = await newTasksRes.json();
